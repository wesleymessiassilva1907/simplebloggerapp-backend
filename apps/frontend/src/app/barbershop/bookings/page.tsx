...
export default function BookingsPage() {
  ...
  const statusColors: Record<string, string> = {
    ...
    'não_compareceu': 'bg-gray-100 text-gray-700',
  };
  ...
  return (
    <DashboardLayout>
      ...
      <div className="card">
        {loading ? <p className="text-center py-8 text-gray-400">Carregando...</p> : <DataTable columns={columns} data={bookings} onEdit={handleEdit} onDelete={handleDelete} />}
      </div>
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title={editItem ? 'Editar Agendamento' : 'Novo Agendamento'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div><label className="block text-sm font-medium text-gray-700 mb-1">Data e Hora *</label><input type="datetime-local" value={form.bookingDate} onChange={e => setForm({...form, bookingDate: e.target.value})} className="input-field" required /></div>
