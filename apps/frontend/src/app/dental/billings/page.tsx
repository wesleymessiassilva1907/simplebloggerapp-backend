...
export default function DentalBillingsPage() {
  ...
  const [form, setForm] = useState({
    patientId: '', description: '', amount: '', status: 'pendente',
    paymentMethod: '', installments: '', dueDate: '',
  });

  const emptyForm = { patientId: '', description: '', amount: '', status: 'pendente', paymentMethod: '', installments: '', dueDate: '' };

  const fetchData = async () => {
    try {
      const [billRes, patRes] = await Promise.all([
        api('/dental/billings?limit=100'),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editItem) {
        await api(`/dental/billings/${editItem.id}`, { method: 'PUT', body: JSON.stringify(data) });
      } else {
        await api('/dental/billings', { method: 'POST', body: JSON.stringify(data) });
      }
  ...
  const handleEdit = (item: any) => {
    setEditItem(item);
    setForm({
      patientId: item.patientId || '', description: item.description || '',
  ...
  const handleDelete = async (item: any) => {
    ...
    try { await api(`/dental/billings/${item.id}`, { method: 'DELETE' }); fetchData(); }
    catch (err: any) { alert(err.message); }
  };
